...
const seed = process.env.SEALED_TIEBAN_V3_SEED || randomBytes(32).toString("hex");
const arena = createSealedTiebanArena({ seed });
const results = [];
for (let index = 0; index < arena.context.sessions.length; index += 1) {
  const descriptor = arena.context.sessions[index];
  results.push(await runInferenceSession({
    descriptor,
  ...
  if ((index + 1) % 250 === 0 || index + 1 === arena.context.sessions.length) {
    process.stdout.write(`\rSealed v3 sessions: ${index + 1}/${arena.context.sessions.length}`);
  }
}
process.stdout.write("\n");

const report = arena.finalize();
const sourceFiles = [
...
const failedGates = Object.entries(report.gates).flatMap(([group, gates]) =>
  Object.entries(gates).filter(([, gate]) => !gate.passed).map(([gate, value]) => ({ group, gate, ...value }))
);
const summary = `# sealed-tieban-v3 实验定稿

## 结论

120候选刻分、60条单命题条文的密封实验完成。预设闸门通过 ${report.gateSummary.passed} 项，失败 ${report.gateSummary.failed} 项；失败项未降低门槛，见下文。
...
const codebooksDirectory = resolve(root, "codebook", "generated");
mkdirSync(reportsDirectory, { recursive: true });
mkdirSync(codebooksDirectory, { recursive: true });
const publicCodebooks = buildPublicCodebooks();
writeFileSync(resolve(reportsDirectory, "canonical-result.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
writeFileSync(resolve(reportsDirectory, "experiment-summary.md"), summary, "utf8");
writeFileSync(resolve(codebooksDirectory, "clause-codebook.json"), `${JSON.stringify(publicCodebooks.clauseCodebook, null, 2)}\n`, "utf8");
writeFileSync(resolve(codebooksDirectory, "candidate-codebook.json"), `${JSON.stringify(publicCodebooks.candidateCodebook, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  sessions: results.length,
  candidates: report.codebooks.candidateCount,
  clauses: report.codebooks.clauseCount,
