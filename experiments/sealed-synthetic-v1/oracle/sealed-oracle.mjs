...
function calibrateThresholds(records, eventCatalog) {
  ...
  const highEligible = scored.filter((item) => item.predicted > 0 && (item.precision ?? 0) >= 0.9);
  const highChoice = [...(highEligible.length ? highEligible : scored)].sort((a, b) => (b.recall ?? -1) - (a.recall ?? -1) || (b.precision ?? -1) - (a.precision ?? -1) || a.threshold - b.threshold)[0];
...
export class SealedOracle {
  ...
  constructor({ cohort, eventCatalog, questionBank, responseSecret = randomBytes(32), modelVersion = "sealed-synthetic-v1.1", minQuestions = 18, maxQuestions = 24 }) {
  ...
  #answer(profile, session, question) {
    ...
    const sensitivity = Math.max(...question.eventIds.map((id) => ({ ordinary: 0, private: 0.05, intense: 0.11 }[this.#eventById[id]?.sensitivity ?? "ordinary"])));
    ...
    const misunderstanding = clamp((1 - persona.comprehension) * (0.65 + question.poolSize * 0.2), 0, 0.7);
  ...
  ask(interviewerToken, sessionId, questionId) {
    ...
    if (question.poolSize !== session.poolSize) throw new OracleError("POOL_SIZE_MISMATCH", "Question pool size is outside this randomized arm.", 403);
  ...
  finalizeAndEvaluate(adminToken) {
    ...
    const thresholds = calibrateThresholds(calibrationRecords, this.#eventCatalog);
